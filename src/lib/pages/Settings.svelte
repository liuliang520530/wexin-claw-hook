...
<section class="card card-hover mt-6 max-w-2xl p-5">
  <h3 class="section-title">Webhook 服务</h3>
  <div class="mt-3 flex items-center gap-3">
    <span class="h-2.5 w-2.5 rounded-full {status?.server_running ? 'bg-ok' : 'bg-line-strong'}"></span>
    <span class="text-sm text-ink-2">{status?.server_running ? `运行中，监听 0.0.0.0:${status.port}` : "已停止"}</span>
    <button class="btn-secondary ml-auto" onclick={toggleServer} disabled={busy}>
  ...
  <div class="mt-4 flex items-end gap-3">
    <label class="text-xs text-ink-3">
      端口
      <input type="number" min="1" max="65535" class="input mt-1 block w-32" bind:value={port} />
    </label>
    <button class="btn-primary" onclick={savePort} disabled={busy}>保存并重启</button>
  </div>
</section>

<section class="card card-hover mt-4 max-w-2xl p-5">
  <h3 class="section-title">API key</h3>
  <p class="mt-1 text-xs text-ink-3">调用 /send 时放在 <code class="font-mono">X-API-Key</code> 头或 <code class="font-mono">Authorization: Bearer</code>。</p>
  <div class="mt-3 flex items-center gap-2">
    <code class="flex-1 truncate rounded-lg bg-panel px-3 py-2 font-mono text-sm text-ink-2">{masked}</code>
    <button class="btn-secondary" onclick={() => (showKey = !showKey)}>{showKey ? "隐藏" : "显示"}</button>
    <button class="btn-secondary" onclick={() => copy(apiKey)}>复制</button>
    <button class={confirmReset ? "btn-confirm" : "btn-secondary"} onclick={resetKey} disabled={busy}>
...
<section class="card card-hover mt-4 max-w-2xl p-5">
  ...
  <table class="mt-4 w-full text-xs">
    <tbody class="text-ink-2">
      ...
      <tr><td class="py-1 pr-3 font-mono text-ink">400</td><td>缺 text / JSON 非法 / 收件人 ID 格式错误或未接入（unknown_recipient）</td></tr>
      <tr><td class="py-1 pr-3 font-mono text-ink">401</td><td>API key 错误</td></tr>
...
{#if message}<p class="alert-ok mt-4 max-w-2xl">{message}</p>{/if}
{#if error}<p class="alert-danger mt-4 max-w-2xl">{error}</p>{/if}
