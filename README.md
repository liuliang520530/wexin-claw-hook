...
把微信 ClawBot（腾讯官方 iLink Bot 协议）包成一个本地 HTTP webhook：扫码登录一次，之后局域网内任何脚本 / curl / CI 一行请求就能给你的微信主动发消息。也支持企业微信自建应用：填一次 corpid / agentid / secret，就能给企业成员推消息。
...
原理很简单：它在本机起了一个 HTTP 服务，你 `POST /send`，它调用微信官方的 iLink Bot API 把文本发到你扫码登录的那个微信上；你 `POST /wecom/send`，它调用企业微信服务端 API，以你的自建应用身份把文本发给企业成员。
...
通过企业微信**自建应用**给企业成员推文本。不需要扫码，凭据是应用的 corpid / agentid / secret，可以添加多个应用（不同企业或同一企业的多个应用），webhook 用 `app` 字段按备注名选应用。
...
- **旧应用（可信 IP 未开启）**：无需额外配置。
- **新应用**：管理后台 → 应用管理 → 打开该应用 → 「企业可信IP」→ 填入本机公网出口 IP。家庭宽带、手机热点的公网 IP 会变（重新拨号、换网络），IP 一变企业微信就会拒绝，返回 errcode `60020`（不安全的访问IP）。在本工具里的表现：添加应用时当场提示「企业微信拒绝了本机 IP（60020）」；运行中 `/wecom/send` 返回 502 `upstream_error`，error 含 `errcode=60020`。到后台把新 IP 加进名单即可恢复。可信 IP 只能在后台手工维护，企业微信没有提供接口自动更新，本工具也不会替你改。
- 想一劳永逸：把本工具放在有固定公网 IP 的机器上跑（如云服务器），可信 IP 配一次就好。

企业微信返回的 60020 错误文案里一般会带上被拒绝的 IP（`from ip: …`），照着填即可；也可以在浏览器里搜「我的 IP」查看本机公网出口 IP。

### 配置步骤

1. 企业微信管理后台 → 应用管理 → 自建 → 创建应用（或用已有应用）。在应用详情页记下 **AgentId** 与 **Secret**（Secret 点「查看」后会发送到管理员的企业微信里）。
2. 「我的企业」→ 企业信息 → 页面底部记下 **企业ID**（即 corpid，形如 `ww` 开头）。
3. 应用的「可见范围」要包含你想通知的成员；不在范围内的成员会出现在响应的 `invalid_users` 里且收不到消息。
4. 新建应用按上一节配好可信 IP。
5. 打开本工具 → 账号页 → 「企业微信」Tab → 添加应用：填 corpid / agentid / secret，备注留空则自动取应用名。点「验证并保存」时会实时调用企业微信校验这三项凭据（顺带也就校验了可信 IP），填错会直接报出具体错误码，不会保存错误的配置。
6. 到「发消息」页的企业微信 Tab 试发一条，或用「设置」页生成的 `/wecom/send` curl 命令。
